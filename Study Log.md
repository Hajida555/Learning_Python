...
1. 《利用python数据分析》
 ...
 - concatenate将数据列表合并
 - np.diff 后一个数 - 前一个数 
 - regplot（） 生成散点图 并添加一条线性回归拟合的直线 
 - sns.pairplot(df拿表格里的每一列其他所有列两两配对，diag_kind=默认对角是直方图可以通过这个参数修改，plot_kws=修改非对角线的图)  
## 1-20
1. 《利用python数据分析》
  - sns.catplot分面网格 即通过不同的筛选分类不同的图 hue只在同一份图中分类 col/row通过不同的子图分类 kind=指定图表类型
  - box箱型图 显示中位数/四分位数/异常值
  - groupby分组再通过函数聚合
  - numeric_only=True 只运算数据类型 对两及以上个键分类获得多层次索引 还可以通过长度正确的任意数组分组
  - size返回分类的分组大小
## 1-21
1. 《利用python数据分析》
 - groupby返回的是可叠加对象 通过使用for使不同的组赋值
 - 多个分组与赋值对象 会依次叠加
 - 还可以通过字典推导式 用一条代码获得相应字典
## 1-22
1. 耶鲁金融
 - 股票收益的反差=贝塔系数的平法乘以市场收益的方差
 - 对于散点图上的直线当直线为与所有散点的垂直距离为最小是-回归线，其中点与线的距离是该点的残差，反过来叫误差
 - 所代表的直线公式y=mx+b，y为股票收益，x为市场收益，m为贝塔系数，b为阿尔法值即Alpha
 - 在散点中往往会出现偏差较大的值即误差值，不利于回归线的正常性
 - 随机变量常常为正态分布，均值即中间值，标准差则反应集中的程度 在金融中误差值会影响分布情况
 - 柯西分布的肥尾两端会存在极端事件造成的极端值
 - 对于两只股票相同概率相同收益，判断是相互独立的，还是相互作用的 通过全概率分解求出协方差 当为零时 相互独立
 - 在投注中需要投注多只低协方差的标的 分散风险
## 1-23
1. 宋浩概率论
 - 随机变量及对每一个值映射一个对应的数，类似于索引
 - 0-1 分布是二项分布的一个特例 x~B(总次数，发生率) 其中当k=[(n+1)p]时，有最大概率，如不为整数则向上取整
 - 泊松分布是离散的，x~P(参数)，通过泊松分布对二项分布做近似取值 当x~B(n,p),n大,n>=100,p小,P<=10时,x~P(BnBp)
 - 几何分布是直到k次才发生，记x~G(概率) 超几何分布 即从总数中取n个，再被除于从小类中取n-k个，可以用二项近似，二项又可以用泊松近似
 - 首先将照几何分布占比小的不放回事件概率看作二项分布的可放回，再用泊松分布近似
 - 分布函数F(x)=P{X<=x}
## 1-24
1. 宋浩概率论
 - 分布函数p{a<x<=b}=F(b)-F(a) ,即中间概率
2. 宋浩高等数学 + 《普林斯顿微积分》
 - 复习微分 可微分必可导 可导必可微分 对于一元函数
 - 对于将函数与x垂直的限定面积 就是定积分
 - 面积可以差分为无数小的矩形面积进行求和 函数图像可分为上和和下和即矩形是否超过函数

## 1-25
 - 变化以后的函数值可以近似于原来的函数值+它的导数乘以德尔塔x即变化的x值
 - 在函数上可导，极大/极小上的导数为零
 - 做T在日内低买高卖/高买低卖，实现降低成本 
 - 早盘9:45-10:15 午盘13:00-13:30 晚盘14:30-15:00
 - 成交与放量大 说明市场积极
 - 线性回归/逻辑回归/比例风险模型类似的都有参数 参数越精确模型与现实越接近 即模型拟合
 - 参数通过最大似然估计 即通过结果预测参数 与分布概率相反
 - 整个数据集的概率便是每个参数自己的概率相乘 再乘以需要的概率模型 例：抛硬币-伯努利 测量误差-正太分布 事件发生的时间-指数函数
 - MLE优点 数据越多 越接近真值 越接近正太分布 同时反差也小 有效性高
 - MLE缺点 依赖样本 有多个局部极值点 同时模型得选对
 - AI 将句拆词(token化) 接着从库/搜索中取概率最高的词 再将词按概率排列 最后随机选择词输出
 - 使用AI可以通过角色设定，输入内容，要求结果 在同一次对话中 相隔一段时间后再问 应该先总结前文内容 